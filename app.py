...
def calculate_price(price: float, quantity: int, coupon: Optional[str]) -> float:
    if DISCOUNT_ENGINE_ENABLED:
        if coupon == "SAVE10":
            return round(price * quantity * 0.9, 2)  # Fixed calculation for SAVE10
        elif coupon == "SAVE50":
            return round(price * quantity - 0.5 * quantity, 2)  # Changed calculation for SAVE50 to apply discount per item
